...
def seo_defaults():
    ...
    path = request.path
    defaults = {
        "title": "Attendance Hub | Student Attendance Management System",
        "description": (
            "Attendance Hub is a Flask-based student attendance management system for admins, "
            "faculty, and students with registration, attendance tracking, approvals, and reports."
        ),
        "keywords": "attendance management system, student attendance app, college attendance software, faculty attendance tracker",
        "robots": "index, follow",
        "canonical_url": absolute_url(path),
        "og_type": "website",
    }
    if endpoint in {"main.login", "main.student_register"}:
        defaults["robots"] = "noindex, nofollow"
    if endpoint == "main.login":
        defaults["title"] = "Login | Attendance Hub"
        defaults["description"] = "Login to Attendance Hub as admin, faculty, or student to manage attendance, reports, and profile workflows."
    elif endpoint == "main.student_register":
        defaults["title"] = "Student Registration | Attendance Hub"
        defaults["description"] = "Create a student account in Attendance Hub with mobile OTP verification, profile details, and secure login access."
    elif endpoint == "main.home":
        defaults["description"] = (
            "Attendance Hub helps schools, colleges, and institutions manage student attendance, edit approvals, applications, and downloadable reports."
        )
    else:
        defaults["robots"] = "noindex, nofollow"
    return defaults
...
def save_user_changes(user_id, form_data, role, allow_password_update=True):
    try:
        update_user_record(user_id, form_data, role, allow_password_update=allow_password_update)
        return None
    except sqlite3.IntegrityError:
        get_db().rollback()
        return "Duplicate data mila. Email, mobile, roll number ya registration number pehle se use ho raha ho sakta hai."
...
def validate_profile_form(form, role, editing_user_id=None):
    ...
    if institution_type and institution_type not in institution_map():
        ...
        errors.append("College ya school name fill karna zaroori hai.")
    ...
    if role == "student" and class_roll_number and not class_roll_number.isdigit():
        errors.append("Class roll number digits me hona chahiye.")
...
def validate_application_file(file_storage):
    ...
    if size > MAX_APPLICATION_SIZE:
        return None, "Application file size 5MB se kam hona chahiye."
...
def save_profile_picture(file_storage, required=False):
    if not file_storage or not file_storage.filename:
        if required:
            return None, "Profile picture upload karna zaroori hai."
        return None, None
    original_name = secure_filename(file_storage.filename)
    extension = Path(original_name).suffix.lower()
    if extension not in ALLOWED_PROFILE_EXTENSIONS:
        return None, "Profile picture sirf JPG, JPEG ya PNG format me honi chahiye."
    file_storage.stream.seek(0, 2)
    size = file_storage.stream.tell()
    file_storage.stream.seek(0)
    if size > MAX_APPLICATION_SIZE:
        return None, "Profile picture size 5MB se kam hona chahiye."
    stored_name = f"{uuid.uuid4().hex}{extension}"
...
def sitemap_xml():
    pages = [
        {"loc": absolute_url(url_for("main.home")), "priority": "1.0", "changefreq": "weekly"},
    ]
    xml_lines = [
...
def login():
    ...
    if request.method == "POST":
        ...
        if role == "student":
            ...
            if role == "admin":
                user = db.execute("SELECT * FROM users WHERE role = ? AND email = ?", (role, identifier)).fetchone()
            ...
            if role != "admin" and institution_name_required(role, institution_type) and not institution_name:
                flash("College ya school choose karna zaroori hai.", "error")
            ...
            if role == "faculty" and not branch:
                flash("Faculty login ke liye branch ya department choose karna zaroori hai.", "error")
                return render_template("login.html", faculty_institutions=available_institutions("faculty", institution_type), student_institutions=available_institutions("student", institution_type))
        ...
        if role == "student":
            flash("Student login failed. Roll number, registration number, email, Gmail ya mobile number ke saath correct password use karein.", "error")
        else:
        ...
        if action == "send_otp":
            if not is_valid_mobile(mobile_number):
                ...
                if existing:
                    ...
                    otp_value = build_otp()
                    db.execute("UPDATE otp_codes SET is_used = 1 WHERE mobile_number = ? AND is_used = 0", (mobile_number,))
                    ...
                    flash("OTP generated. Demo mode mein OTP screen par dikh raha hai.", "success")
        else:
            form = request.form
            ...
            if any(not form.get(field, "").strip() for field in required_fields):
                flash("Sabhi fields bharna zaroori hai. Incomplete form par account create nahi hoga.", "error")
            ...
            if session.get("pending_student_mobile") != form["mobile_number"].strip():
                flash("Pehle same mobile number par OTP request karein.", "error")
                return render_template("student_register.html", otp_value=session.get("pending_student_otp"), registration_mobile=registration_mobile)
            if not otp_row:
            ...
            try:
                ...
                db.rollback()
                flash("Duplicate data mila. Email, mobile, roll number ya registration number pehle se use ho raha ho sakta hai.", "error")
                return render_template("student_register.html", otp_value=session.get("pending_student_otp"), registration_mobile=registration_mobile)
...
def add_student():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            db.rollback()
            flash("Duplicate data mila. Student record create nahi hua.", "error")
            return render_template("add_student.html")
        ...
        try:
            db.execute(
                ...
                (
                    ...
                    form.get("gmail", "").strip() or form["email"].strip(), form["email"].strip(), generate_password_hash(form["password"]), 1, user["id"],
                ),
            )
            db.commit()
        except sqlite3.IntegrityError:
            db.rollback()
            flash("Duplicate data mila. Faculty account create nahi hua.", "error")
            return render_template("add_faculty.html")
...
def student_profile_edit():
    ...
    if request.method == "POST":
        ...
        flash("Profile update request faculty approval ke liye bhej di gayi hai.", "success")
        return redirect(url_for("main.dashboard"))
    return render_template("student_profile_edit.html", student=student)
...
def edit_student(user_id):
    ...
    if viewer["role"] == "faculty" and (
        ...
        flash("Faculty sirf apne college ya school ke students edit kar sakti hai.", "error")
        return redirect(url_for("main.dashboard"))
    if request.method == "POST":
        form_data = request.form.to_dict(flat=True)
        profile_picture, picture_error = save_profile_picture(request.files.get("profile_picture"), required=not bool(student["profile_picture"]))
        ...
        if errors:
            flash(errors[0], "error")
            return render_template("edit_student.html", student=student)
        save_error = save_user_changes(user_id, form_data, "student")
        if save_error:
...
def delete_student(user_id):
    ...
    if user["role"] == "faculty" and (
        ...
        flash("Aap sirf apni branch ke student ko delete kar sakte hain.", "error")
        return redirect(url_for("main.dashboard"))
...
def submit_application():
    ...
    if request.method == "POST":
        ...
        if not application_date or not reason:
            flash("Date aur application reason likhna zaroori hai.", "error")
...
def edit_faculty(user_id):
    ...
    if request.method == "POST":
        ...
        if viewer["role"] == "faculty":
            ...
            flash("Faculty profile update request admin approval ke liye bhej di gayi hai.", "success")
        else:
            save_error = save_user_changes(user_id, form_data, "faculty")
            if save_error:
                flash(save_error, "error")
                return render_template("edit_faculty.html", faculty=faculty)
...
def process_approval(change_id, action):
    ...
    if not approver_can_process(user, change):
        ...
        return redirect(url_for("main.approvals", **redirect_args))
    if action == "approve":
        save_error = save_user_changes(change["target_user_id"], json.loads(change["payload"]), change["target_role"])
        if save_error:
            flash(save_error, "error")
            return redirect(url_for("main.approvals", **redirect_args))
        db = get_db()
